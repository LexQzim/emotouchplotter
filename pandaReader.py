...
def read_and_merge_timeline_data(timelineFilename, sessionDataFilename, delimiter, needed_ids, output_filename = ""):
    timeline_df = pd.read_csv('data/origin/'+timelineFilename+".csv", delimiter=delimiter, encoding="utf-8")
    session_df = pd.read_csv('data/origin/'+sessionDataFilename+".csv", delimiter=delimiter, encoding="utf-8")

    # merge timeline data and session meta data to get soCiSurvey id
    new_timeline_df =  pd.merge(timeline_df[['session_id', 'x', 'created_at_relative', 'type']], session_df[['id','s1']], left_on='session_id', right_on='id')
    # drop rows with nan values
    new_timeline_df = new_timeline_df.dropna()
    # set type of each column for savety reasons
    new_timeline_df = new_timeline_df.astype({'session_id':'int16', 'x':'float32', 'created_at_relative':'float32', 'type':'str', 's1': 'int16'})
    # reformat time from millisecond to second
    new_timeline_df["created_at_relative"] = new_timeline_df["created_at_relative"] / 1000
    # drop rows with negative created at timestamp
    new_timeline_df = new_timeline_df.drop(new_timeline_df[new_timeline_df['created_at_relative'] < 0].index)
    # select only rows of type TICKCHANGE
    new_timeline_df = new_timeline_df.drop(new_timeline_df[new_timeline_df['type'] != 'TICKCHANGE'].index)
    ...
    new_timeline_df = new_timeline_df.drop(new_timeline_df[~new_timeline_df['soSci_survey_Id'].isin(needed_ids)].index)
...
def reorder_timeline_data(timeline_df: pd.DataFrame):
    ...
    '''
    timeline_df = timeline_df.pivot_table(index=['created_at_relative'], columns=['soSci_survey_Id'], values=['x'])
