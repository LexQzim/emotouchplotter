...
# prepare legend
patch_yellow_box = patches.Patch(color='#FFFF0055', edgecolor='#FFFF0055', label='Aufforderung')
patch_noise_plot = patches.Patch(color='#C0C0C055', edgecolor='#C0C0C055', label='Rauschkurve')
song_ends = Line2D([0], [0], color='k', linestyle='dashed', label='Ende vom Liedausschnitt')
rating_origin = Line2D([0], [0],  marker="o", markersize=10, markerfacecolor='#00800055', linestyle='', label='Wertungen (original)')
rating_resampled_line = Line2D([0], [0], color='#008000', linestyle='solid', label='Wertungen (resampled)')
rating_resampled_dot = Line2D([0], [0],  marker="o", markersize=10, markerfacecolor='#00800055', linestyle='', label='Wertungen (resampled)')
...
def _draw_noise_function(noise_version):
    ...
    plt.fill_between(calculated_x_noise_values, calculated_y_noise_values, alpha=0.3, facecolor="silver")
...
def _prepare_plot(title, drawBoxes = False, noise_version = 0):
    '''
...
def create_error_timeline_plot(reordered_timeline_df, noise_version=0, draw_signal_boxes= False):
    '''
    ...
    sns.scatterplot(x='created_at_relative',y='mean', data=reordered_timeline_df, edgecolor = "green", color = "green", alpha = 0.4)

    plt.show()
    plt.close('all')


def create_and_save_scatter_plot(timeline_df, title, output_filename, draw_noise=0, draw_signal_boxes=False, dataResampled = False, testPlot = False):
    '''
    ...
    for i, (soSci_survey_Id, data) in enumerate(timeline_df.groupby('soSci_survey_Id')):
        ...
        sns.scatterplot(x='created_at_relative',y='x', data=data, edgecolor = "green", color = "green", alpha = 0.4)
...
def create_and_save_line_and_scatter_plot(timeline_origin_df, timeline_resampled_df, title, output_filename, draw_noise=0, draw_signal_boxes=False, testPlot = False):    
    '''
    ...
    for i, (soSci_survey_Id, data) in enumerate(timeline_origin_df.groupby('soSci_survey_Id')):
        _prepare_plot(title + str(soSci_survey_Id), draw_signal_boxes, draw_noise)

        sns.scatterplot(x='created_at_relative',y='x', data=data, edgecolor = "green", color = "green", alpha = 0.4)
        sns.lineplot(x='created_at_relative',y='x', data=timeline_resampled_group.get_group(soSci_survey_Id), color = "green")
